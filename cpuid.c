...
static inline int __do_cpuid_func(struct kvm_cpuid_array *array, u32 function)
{
	...
	hus they can be handled by common code.
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	{ 
		if(ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65 || ecx>68 || ecx <0){
			printk(KERN_INFO "Exit reason number of %u is not defined in SDM", ecx);
			eax = 0;
			ebx = 0;
			ecx = 0;
			edx = 0xffffffff; } 
		else if(ecx == 5 || ecx == 6 || ecx == 11 || ecx == 17 || ecx == 35 || ecx == 38 || ecx == 42 || ecx == 66){
			printk(KERN_INFO "Exit reason number %u are not enabled in KVM", ecx);
			eax = ebx = ecx = edx = 0; } 
		else { 
			eax = atomic64_read(&totalExits);
			printk(KERN_INFO "Total number of exits are: %u", eax);  } 
	} 

	else if(eax == 0x4ffffffd) 
	{ 
		if(ecx == 35 || ecx == 38 || ecx == 42 || ecx == 65 || ecx>68 || ecx<0) { 
			printk(KERN_INFO "Exit reason number of %u are not defined in SDM", ecx);
			eax = 0;
			ebx = 0; 
			ecx = 0;
			edx = 0xffffffff;  } 
			...
			printk("High 32 bits of total time spent processing exits in ebx = %d", ebx);
			ecx = (atomic64_read(&totalTime) & 0xFFFFFFFF);
			printk("Low 32 bits of total time spent processing exits in ecx = %d", ecx); 
		} 
	} 
